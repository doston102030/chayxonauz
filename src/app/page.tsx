...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-2xl font-black leading-none">
                ...
                <span className="text-gray-900"> Choyxona</span>
              </h1>
              <p className="text-xs text-gray-400 font-medium">Hisob-kitob tizimi</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            {time && (
              <div className="hidden sm:flex items-center gap-2 bg-gray-100 border border-gray-200 rounded-2xl px-4 py-2">
                <Clock className="w-3.5 h-3.5 text-emerald-600" />
                <span className="font-mono font-bold text-sm tabular-nums text-gray-700">
                  {pad(time.getHours())}:{pad(time.getMinutes())}
                  <span className="text-gray-400 text-xs">:{pad(time.getSeconds())}</span>
                </span>
              </div>
            )}
            <Link href="/admin">
              <button className="flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-green-500 text-white px-5 py-2.5 rounded-2xl font-black text-sm shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 hover:scale-[1.02] transition-all">
          ...
          <div className="max-w-7xl mx-auto px-5 pb-3 flex items-center gap-4">
            <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-1.5">
              ...
              <span className="text-xs font-semibold text-emerald-700">
                <span className="font-black text-emerald-600">{occupied}</span> / {tables.length} band
              </span>
            </div>
            <div className="flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-3 py-1.5">
              <TrendingUp className="w-3.5 h-3.5 text-orange-500" />
              <span className="text-xs font-semibold text-orange-700">
                <span className="font-black text-orange-600">{formatSom(revenue)}</span>
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-xs text-gray-400 font-medium">Jonli</span>
            </div>
          </div>
        )}
      </header>
      ...
      <div className="max-w-7xl mx-auto px-5 py-8">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-40 gap-4">
            <div className="w-14 h-14 rounded-3xl bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center shadow-2xl shadow-emerald-500/30 animate-pulse">
              <span className="text-2xl">☕</span>
            </div>
            <p className="text-gray-400 font-semibold text-sm">Yuklanmoqda...</p>
          </div>
        ) : (
              ...
              return (
                <section key={zone}>
                  ...
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h2 className="font-display font-black text-gray-800 text-lg">{zone}</h2>
                    {zoneOccupied > 0 && (
                      <span className="bg-orange-50 border border-orange-200 text-orange-600 text-xs font-bold px-2.5 py-1 rounded-xl">
                        {zoneOccupied} band
                      </span>
                    )}
                    <div className="flex-1 h-px bg-gradient-to-r from-gray-200 to-transparent" />
                    <span className="text-xs text-gray-400 font-medium">{zoneTables.length} joy</span>
                  </div>
                      ...
                      return (
                        <Link key={table.id} href={`/order/${table.id}`}>
                          ...
                          `}>
                            ...
                            <div className={`font-display font-bold text-sm mb-3 ${isOccupied ? "text-orange-800" : "text-gray-400 group-hover:text-gray-700 transition-colors"}`}>
                              {table.name}
                            </div>

                            {isOccupied ? (
                              <div>
                                <div className="font-black text-orange-600 text-sm">{formatSom(totalSum)}</div>
                                <div className="text-[10px] text-orange-500/70 font-semibold mt-0.5">{itemCount} mahsulot</div>
                              </div>
                            ) : (
                              <div className="text-[11px] text-gray-300 font-semibold">Bo'sh</div>
                            )}
      ...
      <footer className="border-t border-gray-200 mt-16 py-4">
        <div className="max-w-7xl mx-auto px-5 flex justify-between text-xs text-gray-400">
